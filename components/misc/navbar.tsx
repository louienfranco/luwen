...
function SiteHeader() {
  ...
  // Active logic supporting routes + hashes (memoized)
  ...
  const markManualNavWithHref = useCallback(
    (href?: string) => {
      markManualNav();
      if (!href) {
  ...
  // Toggle mobile open state (stable reference)
  const toggleMobileOpen = useCallback(() => setMobileOpen((v) => !v), []);

  // Compute island className once per relevant state changes to avoid
  // allocating a new string on every render.
  ...
  // Render desktop nav items memoized to avoid recreating elements on every
  // parent render. Depend on isActive and hasMounted which change when needed.
        ...
        >
          {active && (
            <motion.span
              ...
              initial={hasMounted ? false : { opacity: 0, scale: 0.97 }}
  ...
  }, [isActive, hasMounted, markManualNavWithHref]);

  // Render mobile nav items memoized
  ...
  }, [isActive, markManualNavWithHref]);
  ...
  return (
    <header className="font-mono pointer-events-none fixed inset-x-0 top-3 z-50 sm:top-4 md:top-6">
      <div className="mx-auto w-full max-w-3xl px-3 sm:px-4 pointer-events-auto">
        ...
        <div ref={islandRef} className={islandClassName}>
          {/* Top bar row */}
          ...
          >
            <div ref={contentRef}>
              <Separator />
              <nav className="px-2 py-2">{mobileNav}</nav>
